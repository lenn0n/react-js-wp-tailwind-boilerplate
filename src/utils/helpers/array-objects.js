...
*/
export function arrayObjectToURLParams(data, selector = "get"){
  if (data.constructor === Array ){ // Array
    ...
    let selections = data.select.map((value) => { return selector + '[]=' + value }).join('&')
    let filters = data?.filter && data.filter.map((value) => { return 'filter' + value }).join('&')
